# -*- coding: utf-8 -*-
"""ML project 6, v2: MRP with MIXTURE religiosity frames (no new data).
...
def main():
    rep = ['MRP v2 — mixture religiosity frames (NNLS anchors + IPF joint)',
           '=' * 66]
    ...
    rep.append('   (v1 all: r=0.780  R2=0.375  MAE=17.1pp)')
    stats(res[res.kind == 'jewish'], 'Jewish SAs')
    rep.append('   (v1 jewish: r=0.744  R2=0.042  MAE=18.3pp)')
    ...
    rep.append('anchors from label-homogeneous SAs (haredi/hiloni sharpened '
               ...
               'tier x acad x age association. Vote model identical to v1.')
    res.to_csv(os.path.join(OUT, 'mrp_v2_sa.csv'), index=False)
    with open(os.path.join(OUT, 'mrp_v2_report.txt'), 'w',
              encoding='utf-8') as f:
        f.write('\n'.join(rep) + '\n')
    print('\n'.join(rep))
