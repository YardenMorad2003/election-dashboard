...
def main():
    ...
    for row in r18:
        ...
        if cfix:
            lat, lng = cfix
            coord_src = "venue"      # corrected precise coordinate
        elif single:
            coord_src = "single_sa"
        else:
    ...
    usable = 0
    for cat in ("precise", "single_sa", "centroid_multi", "noname_multi", "nomatch_multi"):
        print(f"{cat:16s} {cat_ct[cat]:8d} {cat_votes[cat]:11,d} {cat_votes[cat]/geo_votes:7.1%}")
        if cat in ("precise", "single_sa"):
            usable += cat_votes[cat]
    print(f"usable w/o geocoding (precise + single_sa): {usable:,} = {usable/geo_votes:.1%}")
    ...
    for semel, ballots in per_semel.items():
        donors = [x for x in ballots if x["coord_src"] in ("venue", "centroid")
    ...
    for ballots in per_semel.values():
        for x in ballots:
            ...
            if x["coord_src"] in ("venue", "single_sa", "centroid", "neighbor_impute"):
                final_ok += x["kosher"]
